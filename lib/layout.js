...
function splitPages(lines, items, config) {
  ...
  hoistClosingOrnaments(pages, lines);
  return pages;
}

// If a page ends a chapter (next page starts h1, or doc ends) and its tail
// is an ornament block, lift the ornament out of the columns and present it
// as a span-bottom band so columns can balance 50/50 without it.
function hoistClosingOrnaments(pages, allLines) {
  for (let pi = 0; pi < pages.length; pi++) {
    const page = pages[pi];
    const isLastPage = pi === pages.length - 1;
    const nextIsChapter = !isLastPage && pages[pi + 1].spanTop && pages[pi + 1].spanTop.length > 0;
    if (!isLastPage && !nextIsChapter) continue;

    // Find trailing ornament lines from the end of the page's flow.
    const combined = [...page.colA, ...page.colB];
    let tailStart = combined.length;
    while (tailStart > 0 && combined[tailStart - 1].itemType === 'ornament') tailStart--;
    if (tailStart === combined.length) continue; // no trailing ornament

    const tail = combined.slice(tailStart);
    const head = combined.slice(0, tailStart);

    // Re-split head 50/50 into the two cols.
    const total = head.reduce((s, l) => s + l.height, 0);
    let acc = 0, mid = 0;
    for (let k = 0; k < head.length; k++) {
      acc += head[k].height;
      if (acc >= total / 2) { mid = k + 1; break; }
    }
    const newA = head.slice(0, mid);
    const newB = head.slice(mid);

    page.colA = newA;
    page.colB = newB;
    page.spanBottom = tail;
    const eq = computeEqualization(newA, newB, page.effectiveColHeight, /*preH1*/ false);
    page.slackPerGapA = eq.slackA;
    page.slackPerGapB = eq.slackB;
    page.equalizedHeight = eq.target;
  }
}
