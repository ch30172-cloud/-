...
function commonCSS(config) {
  ...
  return `
    ...
    * { box-sizing: border-box; }
    ...
    /* Drop-cap ("חלן"): first word floats, 40px tall, gold-brushed. */
    .doc p.dropcap { text-indent: 0; }
    .doc p.dropcap .dc-word {
      float: right;
      font-family: ${fontStack(f.h1.family)};
      font-weight: 700;
      font-size: ${f.dropCapSizePt || 26}pt;
      line-height: ${config.dropCap.heightPx}px;
      height: ${config.dropCap.heightPx}px;
      margin: 0 0 0 ${config.dropCap.gapPx}px;
      padding: 0;
      background: ${goldGrad};
      -webkit-background-clip: text;
              background-clip: text;
      color: ${config.showGoldOnScreen ? 'transparent' : '#1a1a1a'};
      -webkit-text-fill-color: ${config.showGoldOnScreen ? 'transparent' : '#1a1a1a'};
    }
    /* Line 2 hanging indent: simulated via padding on the wrapper's flow. */
    .doc p.dropcap.line2-hang::first-line { /* no-op shim */ }
